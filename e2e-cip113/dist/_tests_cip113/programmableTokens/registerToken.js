 ...
 */
import { MeshTxBuilder, deserializeDatum, applyParamsToScript, stringToHex, byteString, conStr0, conStr1, integer, } from "@meshsdk/core";
import { blockchainProvider, wallet1, wallet1VK, wallet1Collateral, } from "../setup.js";
import { BLACKLIST_MINT_HASH, TOKEN_POLICY_ID, TOKEN_ASSET_NAME, TOKEN_SUPPLY, registryMintPolicyId, issuanceScriptHash, issuanceCbor, issuancePolicyId, transferLogicHash, adminContractCbor, adminContractHash, adminContractRewardAddr, wallet1SmartAddr, NETWORK_ID, protocolParamsPolicyId, getValidator, validateConfig, } from "./config.js";
validateConfig();
...
const headNodeAddresses = await blockchainProvider.fetchAssetAddresses(registryMintPolicyId);
...
const issuanceCborHexAddresses = await blockchainProvider.fetchAssetAddresses(issuanceCborHexUnit);
if (!issuanceCborHexAddresses?.length)
    throw new Error(`IssuanceCborHex asset not found on chain. Unit: ${issuanceCborHexUnit}`);
const issuanceCborHexUtxos = await blockchainProvider.fetchAddressUTxOs(issuanceCborHexAddresses[0].address);
...
const protocolParamsUtxos = await blockchainProvider.fetchAddressUTxOs(protocolParamsAddresses[0].address);
...
const registrySpendCbor = applyParamsToScript(getValidator("registry_spend.registry_spend.spend"), [byteString(protocolParamsPolicyId)], "JSON");
...
txBuilder
    ...
    .mintRedeemerValue(conStr0([conStr1([byteString(adminContractHash)])]), "JSON");
