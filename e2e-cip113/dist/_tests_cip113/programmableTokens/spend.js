 ...
 */
import { MeshTxBuilder, applyParamsToScript, deserializeDatum, stringToHex, byteString, conStr, conStr0, integer, list, } from "@meshsdk/core";
import { blockchainProvider, wallet1, wallet1VK, wallet1Collateral, } from "../setup.js";
import { BLACKLIST_MINT_HASH, TOKEN_POLICY_ID, TOKEN_ASSET_NAME, globalRewardAddr, baseCbor, wallet1SmartAddr, getSmartAddr, blacklistSpendAddr, transferLogicCbor, transferLogicHash, transferLogicRewardAddr, registrySpendAddr, protocolParamsPolicyId, NETWORK_ID, getValidator, validateConfig, } from "./config.js";
validateConfig();
...
// ─────────────────────────────────────────────────────────────────────────────
console.log("\n=== Smart Wallet UTxOs ===");
const smartWalletUtxos = await blockchainProvider.fetchAddressUTxOs(senderSmartAddr);
...
for (const spendUtxo of selectedUtxos) {
    ...
    console.log(`✅ Proof node: ${coveringNode.input.txHash.slice(0, 8)}...#${coveringNode.input.outputIndex}`);
    console.log(`   range: "${selectedKey || "(origin)"}" < "${senderProofKeyHash}" < "${selectedNext}"`);
}
...
const proofIndices = proofs.map((p) => sortedRefs.findIndex((r) => r.txHash === p.blacklistUtxo.input.txHash &&
    r.outputIndex === p.blacklistUtxo.input.outputIndex));
console.log(`Registry index: ${registryIndex}, Proof indices: [${proofIndices}]`);
// ─────────────────────────────────────────────────────────────────────────────
// Redeemers
