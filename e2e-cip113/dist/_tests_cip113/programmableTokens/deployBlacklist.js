 ...
 */
import { MeshTxBuilder, applyParamsToScript, resolveScriptHash, serializePlutusScript, byteString, conStr0, conStr, integer, } from "@meshsdk/core";
...
const seedUtxo = walletUtxos.find((u) => u.output.amount.length === 1 && u.output.amount[0].unit === "lovelace") ?? walletUtxos[0];
if (!seedUtxo)
    throw new Error("No UTxOs available in wallet1");
...
const blacklistSpendCbor = applyParamsToScript(getValidator("blacklist_spend.blacklist_spend.spend"), [byteString(blacklistMintHash)], "JSON");
