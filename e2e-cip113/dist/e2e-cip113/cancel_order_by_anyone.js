import { MeshTxBuilder, SLOT_CONFIG_NETWORK, conStr, conStr0, deserializeAddress, deserializeDatum, integer, list, mConStr2, serializeAddressObj, stringToHex, unixTimeToEnclosingSlot, } from "@meshsdk/core";
import { NETWORK_ID, baseScript, blockchainProvider, orderCanclValidatorRewardAddress, orderCanclValidatorScript, orderValidatorAddress, orderValidatorRewardAddress, orderValidatorScript, orderValidatorScriptHash, wallet1, wallet1Address, wallet1Collateral, wallet1Utxos, } from "./setup.js";
...
const protocolParamsUtxo = protocolParamsUtxos.find((utxo) => utxo.output.amount.some((asset) => asset.unit === protocolParamsUnit));
...
let globalRedeemer = conStr0([list([])]);
let transferRedeemer = list([]);
if (hasPurrfluidTokens) {
    ...
    refs.push(...uniqueWhitelistUtxos(proofs).map((utxo) => ({
    ...
    const registryIndex = refs.findIndex((ref) => ref.txHash === registryNode.input.txHash &&
        ref.outputIndex === registryNode.input.outputIndex);
    const proofIndices = proofs.map((proof) => refs.findIndex((ref) => ref.txHash === proof.utxo.input.txHash &&
