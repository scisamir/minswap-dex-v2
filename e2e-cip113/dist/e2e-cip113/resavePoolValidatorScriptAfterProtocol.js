import { applyCborEncoding, MeshTxBuilder, resolveTxHash, serializePlutusScript, stringToHex, } from "@meshsdk/core";
import { blockchainProvider, poolScriptTxHash, poolScriptTxIndex, poolValidatorScript, wallet1, wallet1Address, wallet1Collateral, wallet1Utxos, } from "./setup.js";
import { NETWORK_ID, protocolParamsPolicyId } from "./purrfluid/config.js";
const refHolderScript = applyCborEncoding("5857010100323232323225333002323232323253330073370e900118041baa00113233224a060160026016601800260126ea800458c024c02800cc020008c01c008c01c004c010dd50008a4c26cacae6955ceaab9e5742ae89");
...
const compareTxHash = (left, right) => left.toLowerCase().localeCompare(right.toLowerCase());
const lovelaceOf = (utxo) => BigInt(utxo.output.amount.find((asset) => asset.unit === "lovelace")?.quantity ??
    "0");
const isSameUtxo = (left, right) => left.input.txHash === right.input.txHash &&
    left.input.outputIndex === right.input.outputIndex;
const oldPoolRefUtxo = (await blockchainProvider.fetchUTxOs(poolScriptTxHash, poolScriptTxIndex))[0];
...
const protocolParamsUtxo = protocolParamsAddressUtxos.find((utxo) => utxo.output.amount.some((asset) => asset.unit === protocolParamsUnit));
...
if (!oldPoolRefUtxo) {
    throw new Error(`Current pool ref-script UTxO not found: ${poolScriptTxHash}#${poolScriptTxIndex}`);
}
if (oldPoolRefUtxo.output.address !== refHolderAddress) {
    throw new Error(`Pool ref-script UTxO is not at the expected ref holder address: ${oldPoolRefUtxo.output.address}`);
}
const minFundingLovelace = BigInt(process.env.MIN_FUNDING_LOVELACE ?? "50000000");
const maxFundingInputs = Number(process.env.MAX_FUNDING_INPUTS ?? "6");
const spendableWalletUtxos = wallet1Utxos
    .filter((utxo) => utxo.output.address === wallet1Address &&
...
console.log("Old pool ref script:", `${oldPoolRefUtxo.input.txHash}#${oldPoolRefUtxo.input.outputIndex}`);
console.log("Fee UTxOs:", feeUtxos.map((utxo) => `${utxo.input.txHash}#${utxo.input.outputIndex}:${lovelaceOf(utxo)} assets=${utxo.output.amount.length}`));
console.log("Fee UTxO total:", feeTotal.toString(), "target:", minFundingLovelace.toString());
if (feeTotal < minFundingLovelace) {
    console.warn(`Warning: selected funding UTxOs are below target. Increase MAX_FUNDING_INPUTS or add a larger wallet UTxO if selection still fails.`);
}
...
for (let nonce = startNonce; nonce < startNonce + maxAttempts; nonce += 1) {
    ...
    for (const utxo of feeUtxos) {
        tx = tx.txIn(utxo.input.txHash, utxo.input.outputIndex, utxo.output.amount, utxo.output.address, utxo.output.scriptRef ? utxo.output.scriptRef.length / 2 : 0);
    }
    ...
    console.log(`nonce=${nonce} txHash=${txHash} ${isAfterProtocolParams ? "OK" : "too early"}`);
    if (!isAfterProtocolParams)
        continue;
    if (!shouldSubmit) {
        console.log("\nCandidate found but not submitted.");
        console.log(`To submit this exact nonce, run: SUBMIT=1 START_NONCE=${nonce} MAX_ATTEMPTS=1 node ./dist/e2e-cip113/resavePoolValidatorScriptAfterProtocol.js`);
        process.exit(0);
    }
...
throw new Error(`No candidate tx hash after ProtocolParams found in ${maxAttempts} attempts. Increase MAX_ATTEMPTS.`);
