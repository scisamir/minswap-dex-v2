 ...
 */
import { BlockfrostProvider, MeshTxBuilder, byteString, conStr0, conStr1, deserializeAddress, deserializeDatum, } from "@meshsdk/core";
import { blockchainProvider, wallet1, wallet1VK, wallet1Collateral, } from "../setup.js";
import { NETWORK_ID, whitelistMintCbor, whitelistPolicyId, whitelistSpendAddr, whitelistSpendCbor, validateConfig, } from "./config.js";
validateConfig();
...
const nodeTokenUnit = (key) => whitelistPolicyId + key;
...
for (const utxo of whitelistUtxos) {
    ...
    try {
        ...
        const hasNodeToken = utxo.output.amount.some((a) => a.unit === nodeTokenUnit(key) && a.quantity === "1");
        if (!hasNodeToken)
            continue;
...
console.log(`Covering node: ${coveringUtxo.input.txHash}#${coveringUtxo.input.outputIndex}`);
console.log(`range: "${coveringNode.key || "(origin)"}" < target < "${coveringNode.next}"`);
