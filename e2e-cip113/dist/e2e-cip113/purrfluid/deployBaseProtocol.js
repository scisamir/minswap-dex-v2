...
const submitTx = async (label, signedTx) => {
    try {
...
const stripCborBytestringHeader = (hex) => {
    ...
    if (first === 0x5a)
        return hex.slice(10);
    throw new Error(`Unexpected CBOR bytestring header: 0x${first.toString(16)}`);
};
const deriveIssuanceTemplate = () => {
    const issuanceCompiled = getBaseValidator("issuance_mint.issuance_mint.mint");
    const hashA = "11".repeat(28);
    const hashB = "22".repeat(28);
    const scriptA = stripCborBytestringHeader(applyParamsToScript(issuanceCompiled, [
        scriptCredential(hashA),
        byteString(hashA),
        scriptCredential(hashA),
        scriptCredential(hashA),
    ], "JSON"));
    const scriptB = stripCborBytestringHeader(applyParamsToScript(issuanceCompiled, [
    ...
    let prefixLength = 0;
    while (prefixLength + 2 <= scriptA.length &&
        scriptA.slice(prefixLength, prefixLength + 2) ===
            scriptB.slice(prefixLength, prefixLength + 2)) {
        prefixLength += 2;
    }
    let suffixLength = 0;
    while (suffixLength + 2 <= scriptA.length - prefixLength &&
        scriptA.slice(scriptA.length - suffixLength - 2, scriptA.length - suffixLength) ===
            scriptB.slice(scriptB.length - suffixLength - 2, scriptB.length - suffixLength)) {
        suffixLength += 2;
    }
...
if (!seedUtxo)
    throw new Error("No pure ADA seed UTxO found in wallet1");
...
const alwaysFailCbor = applyParamsToScript(getBaseValidator("always_fail.always_fail.spend"), [byteString(seedTxHash)], "JSON");
...
const protocolParamsMintCbor = applyParamsToScript(getBaseValidator("protocol_params_mint.protocol_params_mint.mint"), [seedRef, byteString(alwaysFailHash)], "JSON");
const protocolParamsPolicyId = resolveScriptHash(protocolParamsMintCbor, "V3");
const issuanceCborHexMintCbor = applyParamsToScript(getBaseValidator("issuance_cbor_hex_mint.issuance_cbor_hex_mint.mint"), [seedRef, byteString(alwaysFailHash)], "JSON");
const issuanceCborHexPolicyId = resolveScriptHash(issuanceCborHexMintCbor, "V3");
const globalCbor = applyParamsToScript(getBaseValidator("programmable_logic_global.programmable_logic_global.withdraw"), [byteString(protocolParamsPolicyId)], "JSON");
const globalHash = resolveScriptHash(globalCbor, "V3");
const baseCbor = applyParamsToScript(getBaseValidator("programmable_logic_base.programmable_logic_base.spend"), [scriptCredential(globalHash)], "JSON");
const baseHash = resolveScriptHash(baseCbor, "V3");
const registrySpendCbor = applyParamsToScript(getBaseValidator("registry_spend.registry_spend.spend"), [byteString(protocolParamsPolicyId)], "JSON");
const registrySpendHash = resolveScriptHash(registrySpendCbor, "V3");
const registrySpendAddr = serializeAddressObj(scriptAddress(registrySpendHash), NETWORK_ID);
const registryMintCbor = applyParamsToScript(getBaseValidator("registry_mint.registry_mint.mint"), [
    seedRef,
...
console.log("txProvider:             ", blockfrostId ? "Blockfrost" : "Maestro");
...
const usedBootstrapInputs = new Set(bootstrapTxBuilder.meshTxBuilderBody.inputs.map((input) => `${input.txIn.txHash}#${input.txIn.txIndex}`));
const refScriptWalletUtxos = selectableWalletUtxos.filter((utxo) => !usedBootstrapInputs.has(`${utxo.input.txHash}#${utxo.input.outputIndex}`));
if (refScriptWalletUtxos.length === 0) {
    throw new Error("Bootstrap transaction used every available wallet UTxO. Fund wallet1 or wait for change before saving reference scripts.");
}
