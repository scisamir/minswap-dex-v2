 ...
 */
import { BlockfrostProvider, MeshTxBuilder, deserializeDatum, stringToHex, conStr, conStr0, integer, list, } from "@meshsdk/core";
...
import { TOKEN_POLICY_ID, TOKEN_ASSET_NAME, globalCbor, globalRewardAddr, baseCbor, wallet1SmartAddr, getSmartAddr, whitelistPolicyId, whitelistSpendAddr, transferLogicCbor, transferLogicHash, transferLogicRewardAddr, registrySpendAddr, protocolParamsPolicyId, NETWORK_ID, validateConfig, } from "./config.js";
validateConfig();
...
const registryThirdPartyHash = registryDatum?.fields?.[3]?.fields?.[0]?.bytes ?? "";
...
if (registryTransferHash && registryTransferHash !== transferLogicHash) {
    throw new Error(`Config mismatch: registry transfer hash is ${registryTransferHash}, but local transferLogicHash is ${transferLogicHash}. ` +
...
const protocolParamsAddressUtxos = await txProvider.fetchAddressUTxOs(protocolParamsAddresses[0].address);
...
// ─────────────────────────────────────────────────────────────────────────────
console.log("\n=== Smart Wallet UTxOs ===");
const smartWalletUtxos = await txProvider.fetchAddressUTxOs(senderSmartAddr);
...
for (const keyHash of proofKeys) {
    const membershipNode = whitelistUtxos.find((wlUtxo) => {
        if (!wlUtxo.output.plutusData)
            return false;
        try {
            const hasWhitelistToken = wlUtxo.output.amount.some((a) => a.unit === whitelistPolicyId + keyHash && a.quantity === "1");
    ...
    if (!membershipNode)
        throw new Error(`No whitelist membership node for key hash: ${keyHash}\n` +
            `whitelistSpendAddr: ${whitelistSpendAddr}`);
    proofs.push({ key: keyHash, whitelistUtxo: membershipNode });
    console.log(`Proof node for ${keyHash}: ${membershipNode.input.txHash.slice(0, 8)}...#${membershipNode.input.outputIndex}`);
}
// Deduplicate whitelist nodes
const uniqueWhitelistUtxos = [];
const seen = new Set();
...
sortedRefs.forEach((r, i) => console.log(`  [${i}] ${r.label}: ${r.txHash}#${r.outputIndex}`));
...
const proofIndices = proofs.map((p) => sortedRefs.findIndex((r) => r.txHash === p.whitelistUtxo.input.txHash &&
