...
const isRegistered = async (address: string) => {
  try {
    ...
    const status =
      ...
      (error as { data?: { status_code?: number } })?.data?.status_code;
    const errorText = (() => {
...
for (const account of rewardAccounts) {
  ...
  console.log(
    `${account.label}: ${registered ? "already registered" : "needs registration"}`
  );
  if (!registered) missingAccounts.push(account);
}
...
console.log(
  "Registered:",
  missingAccounts.map((account) => account.label).join(", ")
);
