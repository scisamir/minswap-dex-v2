...
for (const proof of proofNodes) {
  console.log(
    `proof witness ${proof.utxo.input.txHash}#${proof.utxo.input.outputIndex}: ${proof.witnessKey}`
  );
}

const uniqueBlacklistNodes: typeof blacklistUtxos = [];
const seenBlacklistNodes = new Set<string>();
for (const proof of proofNodes) {
...
const proofIndices = proofNodes.map((proof) =>
  sortedRefInputs.findIndex(
    (r) =>
      r.txHash === proof.covering.input.txHash &&
