...
const maestroKey = getNetworkEnv("MAESTRO_KEY");
if (!maestroKey) {
  throw new Error(`MAESTRO_KEY_${NETWORK.toUpperCase()} does not exist`);
}
...
const wallet1Utxos = (await wallet1.getUtxos()).filter(
  (utxo) => !utxo.output.scriptRef
);
// const wallet1Collateral: UTxO = (
//   await blockchainProvider.fetchUTxOs(
//     "efefc4e3f392d3afc2f3fa4a3cd6cb36e15ab3b7b3e12c7a5be73f19d3c1985b",
//     1
//   )
// )[0];
...
const alwaysSuccessValidatorHash = resolveScriptHash(
...
const dexInitParamTxHash =
  "8ce502a78186d611c17891157355645f43ef3a7f8d1768bbb0c7adec2e82d9d2"; // change this and below on each dex init
const dexInitParamTxIndex = 0;
...
const poolScriptHash = poolStakeCredentialHash;
...
const assetA = mConStr0([alwaysSuccessMintValidatorHash, tokenA]);
